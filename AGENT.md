...
- Permission rules: user-defined allow/deny rules (glob per Pi tool) that overlay the permission modes. Deny beats allow beats mode default; deny applies in every mode. Global rules live in `<GUI data dir>/permission-rules.json`; a workspace `.pi-desktop/permission-rules.json` fully replaces them (one-time in-app notice per workspace, pointing at the workspace tab). Settings → Behavior edits BOTH scopes via Global | This workspace tabs: create, edit, and remove workspace rules (in-app danger confirm), Copy from global (seeds an unsaved draft from the current global list), and per-scope JSON import/export. Manual editing of either file on disk remains fully supported — switching scope tabs re-reads that file, so hand-edited rules show up without a restart. Engine: `resources/permission-rules.ts`, shared by the Pi extension (jiti relative import, mtime-cached live re-read) and the main process. The permissions extension always loads alongside Pi when present on disk, regardless of mode or whether rules currently exist, so a rules file created mid-session is enforced immediately rather than after a restart.
  - Trust posture: a workspace's `.pi-desktop/permission-rules.json` is trusted repo content, not GUI-managed config — cloning an untrusted repo means trusting its rules file. Its allow rules suppress ask-mode prompts for that workspace, and the one-time notice dialog is informational, not a consent gate. Rule globs match raw tool input strings only (no path canonicalization, no command parsing), so rules are a guardrail against accidents, not a security sandbox.
